...
<div class="clear"></div>
<div class="info">作者：<?php the_author() ?> | 发布：<?php the_time('Y-m-d H:i') ?> | 分类：<?php the_category(', ') ?> <?php if(function_exists('the_views')) { echo '| 阅读：'; the_views(); } ?> <?php the_tags('| 标签：', ', ', ''); ?></div><div class="comments_num"><?php comments_popup_link ('无评论','1条评论','%条评论'); ?></div>
